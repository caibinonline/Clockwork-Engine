...
namespace clockwork {
	namespace utils {
		...
		struct Chunk
		{
			...
			maths::Vec3<int> m_id;
			ChunkSystem* m_ChunkSystem;//vielleicht wird dieses nicht gebraucht, da über state auf ChunkSystem2 zugegriffen werden kann und hier ggf state gespeichert werden muss
			...
			void init(const maths::Vec3<float>& min, const maths::Vec3<float>& max, const maths::Vec3<int>& id, ChunkSystem* ChunkSystem) noexcept;
		...
		struct ChunkSystem
		{
			...
			ChunkSystem(const maths::Vec3<float>& min, const maths::Vec3<float>& max, const maths::Vec3<float>& chunksize) noexcept///das hier alles optimieren ohne temporäre objekte, indem die sachen direkt berechnet werden
		...
		void Chunk::init(const maths::Vec3<float>& min, const maths::Vec3<float>& max, const maths::Vec3<int>& id, ChunkSystem* ChunkSystem) noexcept
		{
			m_min = min;
			m_max = max;
			m_id = id;
